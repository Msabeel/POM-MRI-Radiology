...
class FuseUtils {
	...
	static setRoutes(config, defaultAuth, USERPermissions) {
		let routes = [...config.routes];
		routes = routes.map(route => {
			let permission = config.permission || config.permission === null ? config.permission : '' || null;
			permission = route.permission || route.permission === null ? route.permission : '' || permission;
	...
	static generateRoutesFromConfigs(configs, defaultAuth) {
		let allRoutes = [];
		let USERPermissions = window.localStorage.getItem('USERPermissions');
		if(USERPermissions) {
			USERPermissions = JSON.parse(USERPermissions);
		}
		configs.forEach(config => {
			allRoutes = [...allRoutes, ...this.setRoutes(config, defaultAuth, USERPermissions)];
		});
		return allRoutes;
	}
	...
	static hasPermission(authArr, userRole, permission, item) {
		/**
		 * If auth array is not defined
		 * Pass and allow
		 */
		if(item === undefined || item.path === "/" || item.path === "/login" || userRole.indexOf('patient') >= 0) {
			return true;
		}

		if(item.path && (item.path.indexOf('/pages/patient-portal') >= 0 || item.path.indexOf('/pages/paper-portal') >= 0)) {
			return true;
		}

		if(permission && permission.permissionData) {
			if(item.permission === undefined || item.permission === null) {
				return true;
			}
			const a = permission.permissionData.some(r => item.permission.indexOf(r.permission_name) >= 0);
			const usertype = window.self !== window.top;//window.localStorage.getItem('usertype');
			return a || usertype;
		}
		else {
			return false;
	...
	static hasButtonPermission(permission_name) {
		let USERPermissions = window.localStorage.getItem('USERPermissions');
		if(USERPermissions) {
			USERPermissions = JSON.parse(USERPermissions);
		}
		if(USERPermissions !== undefined && USERPermissions.permissionData) {
			const a = USERPermissions.permissionData.some(r => permission_name.indexOf(r.permission_name) >= 0);
			const usertype = window.self !== window.top;//window.localStorage.getItem('usertype');
			return a || usertype;
		}
	}
