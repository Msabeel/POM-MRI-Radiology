...
import { getPatientByEmail, openLoginPatientsDialog } from '../store/loginSlice';
...
function PatientForm() {
    ...
    const handleSubmit = async event => {
        ...
        const data = await dispatch(getPatientByEmail(values));
        const patients = data.payload.data;
        if(patients && patients.length === 1) {
            const result = await dispatch(submitPatientLoginWithAWS({...values, patient_id: patients[0].patient_id }, locationdata));
            setValues({ ...values, loading: false, result });
            if(!result.error) {
                history.push({
                    pathname: '/pages/patient-page/' + patients[0].id + '/'
                });
            }
        }
        else {
            setValues({ ...values, loading: false });
            dispatch(openLoginPatientsDialog({ patients, values }))
        }
    };
    ...
    return (
        <form className={classes.root} noValidate autoComplete="off" onSubmit={handleSubmit}>
            ...
            {(values.result.error || LoginPatientsDialog.error) && 
