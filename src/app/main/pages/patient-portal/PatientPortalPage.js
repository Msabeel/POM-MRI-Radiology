...
import {getPatientPortalData, generateReports, requestAccess, generateImages, openConfirmationDialog} from './store/patientPortalSlice';
...
function PatientPortalPage() {
	...
	useEffect(() => {
		async function fetchPatientPortalData() {
			setLoadingPAge(true);
			await dispatch(getPatientPortalData(routeParams));
	...
	useEffect(() => {
		async function fetchPatientPortalData() {
			setLoadingPAge(true);
			const res = await dispatch(sendPatientAccessMail({id: patientId, actionType: ['login'], "emailIds": [], "faxNos": [], "request_for": "patient"}));
			await dispatch(getPatientPortalData({token: res.payload.data.token}));
			setLoadingPAge(false);
		}
		if (patientId) {
			fetchPatientPortalData();
	...
	useEffect(() => {
		function getFilteredArray() {
			if (searchText.length === 0) {
				return patientDetails.Allexams;
			}
			return _.filter(patientDetails.Allexams, item => {
				return item.exam && item.exam.toLowerCase().includes(searchText.toLowerCase());
			});
		}

		if (patientDetails.Allexams) {
			setFilteredData(getFilteredArray());
		}
	}, [patientDetails.Allexams, searchText]);

	useEffect(() => {
		function setSelectedExam() {
			const selectedExam = [];
			patientDetails.Allexams.map((item, i) => {
				if (i < 2) {
	...
	async function handleDownloadClick() {
		dispatch(openConfirmationDialog({id: patientDetails.patient_data.id, examIDs: state.selectedExam, "isDownloadReport": true, "isDownloadImage": false}));
		// setLoading(true);
	...
	async function handleDownloadImageClick() {
		...
		dispatch(openConfirmationDialog({id: patientDetails.patient_data.id, examIDs: state.selectedExam, "isDownloadReport": false, "isDownloadImage": true, token}));
	...
	async function handleDownloadDocumentsClick() {
		dispatch(openConfirmationDialog({
			...
			id: patientDetails.patient_data.id,
