...
class MultiColumnRow extends React.Component {
  render() {
    const {
      controls, data, editModeOn, getDataById, setAsChild, removeChild, seq, className, index, subClass
    } = this.props;
...
const TwoColumnRow = ({ data, class_name, ...rest }) => {
  const className = class_name || 'w-1/2 mr-10';
  if (!data.childItems) {
...
const ThreeColumnRow = ({ data, class_name, ...rest }) => {
  const className = class_name || 'w-1/3 mr-10';
  if (!data.childItems) {
    // eslint-disable-next-line no-param-reassign
...
const GroupRow = ({ data, class_name, ...rest }) => {
  const className = class_name || "col-md-3 m-10";
  if (!data.childItems) {
    // eslint-disable-next-line no-param-reassign
    data.childItems = [null, null, null, null];
    data.isContainer = true;
  }
  return <MultiColumnRow {...rest} className={className} data={data} subClass="flex-col" />;
};

export { TwoColumnRow, ThreeColumnRow, FourColumnRow, GroupRow };
