...
function LOPAttorneyInfo(props) {
    ...
    const initDialog = useCallback(async () => {
		if (lopCoverageInfoDialog.data) {
			setForm({ ...lopCoverageInfoDialog.data });
		}
	}, [lopCoverageInfoDialog.data, setForm]);

	useEffect(() => {
		/**
		 * After Dialog Open
		 */
		if (lopCoverageInfoDialog.props.open) {
			initDialog();
		}
	}, [lopCoverageInfoDialog.props.open, initDialog]);
	...
	function closeComposeDialog() {
		dispatch(closeLOPCoverageDialog(form));
	}
    ...
    function handleactive_coverage2(event, newValue) {
		setForm({ 
			...form,
			active_coverage2: newValue
		});
	}
	...
	function handleWorkerMedPay(event, newValue) {
		setForm({ 
			...form,
			lop_med_pay: newValue
		});
	}

	function handleBenefitRemaining(event, newValue) {
		setForm({ 
			...form,
			lop_benefit_remaining: newValue
		});
	}
	...
	function handleDeductableMeetAmount1(event) {
		...
		if(parseFloat(str) > parseFloat(form.deductable_amount1)) {
			setValidationErrors({...validationErrors, lop_deductable_meet_amount: 'Deductible Paid should be less then Plan Deductible Amount' });
		}
		else {
			setValidationErrors({ });
		}
		setForm({ 
			...form,
			lop_deductable_meet_amount: str
		});
	}
			...
			<AppBar position="static" elevation={1}>
				<Toolbar className="flex w-full">
					<Typography variant="subtitle1" color="inherit">
						LOP Insurance {'>'} Coverage & Deductible Detail
					</Typography>
				</Toolbar>
			</AppBar>
			<Formsy
            ...
            >
				<DialogContent classes={{ root: 'p-24' }}>
					<div className="flex mb-24">
                        ...
                        <div className="w-1/2">
                            <StyledGroupButton
                                value={form.active_coverage2}
                                id="active_coverage2"
                                name="active_coverage2"
                                exclusive
                                onChange={handleactive_coverage2}
                                aria-label="active_coverage2"
                                >
					...
					{form.active_coverage2 === "Yes" && (
					<div className="flex mb-24">
						<TextFieldFormsy
                            id="coverage_terminated_date2"
                            name="coverage_terminated_date2"
                            label="Coverage Terminated Date"
                            type="date"
                            value={moment(form.coverage_terminated_date2).format("YYYY-MM-DD")}
					...
					<div className="flex mb-24">
						<TextFieldFormsy
							type="text"
							name="lop_deductable_amount"
							id="lop_deductable_amount"
							value={form.lop_deductable_amount}
							onChange={handleChange}
					...
					<div className="flex mb-24">
						...
						<div className="w-1/2">
							<StyledGroupButton
								value={form.lop_deductable_meet}
								id="lop_deductable_meet"
								name="lop_deductable_meet"
								exclusive
								onChange={handleDeductableMeet1}
								aria-label="lop_deductable_meet"
								>
								<StyledToggleButton value="Yes" aria-label="left aligned">
									Yes
								</StyledToggleButton>
					...
					{form.active_coverage2 === "Yes" && form.lop_deductable_meet === "No" && (
					<div className="flex mb-24">
						<TextFieldFormsy
							type="text"
							name="lop_deductable_meet_amount"
							id="lop_deductable_meet_amount"
							value={form.lop_deductable_meet_amount}
							...
							validations={{
								matchRegexp: floatRegExp,
								// checkValue: form.lop_deductable_meet_amount < form.deductable_amount1
							}}
							validationErrors={{
								matchRegexp: floatRegExpMessage
							}}
					...
					<div className="flex mb-24">
						<TextFieldFormsy
							...
							value={form.lop_deductable_amount - form.lop_deductable_meet_amount}
							onChange={handleChange}
					...
					<div className="flex mb-24">
						...
						<div className="w-1/2">
							<StyledGroupButton
								value={form.lop_med_pay}
								id="lop_med_pay"
								name="lop_med_pay"
								exclusive
								onChange={handleWorkerMedPay}
								aria-label="lop_med_pay"
								>
								<StyledToggleButton value="Yes" aria-label="left aligned">
									Yes
								</StyledToggleButton>
					...
					<div className="flex mb-24">
						...
						<div className="w-1/2">
							<StyledGroupButton
								value={form.lop_benefit_remaining}
								id="lop_benefit_remaining"
								name="lop_benefit_remaining"
								exclusive
								onChange={handleBenefitRemaining}
								aria-label="lop_benefit_remaining"
								>
								<StyledToggleButton value="Yes" aria-label="left aligned">
									Yes
								</StyledToggleButton>
...
export default LOPAttorneyInfo;
