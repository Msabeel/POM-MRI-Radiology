...
function LOPDeductibleInfo(props) {
    ...
    const initDialog = useCallback(async () => {
		if (lopDeductibleInfoDialog.data) {
			setForm({ ...lopDeductibleInfoDialog.data });
		}
	}, [lopDeductibleInfoDialog.data, setForm]);

	useEffect(() => {
		/**
		 * After Dialog Open
		 */
		if (lopDeductibleInfoDialog.props.open) {
			initDialog();
		}
	}, [lopDeductibleInfoDialog.props.open, initDialog]);

	function closeComposeDialog() {
		dispatch(closeLOPDeductibleDialog(form));
	}
	...
	function handleDeductableMeetAmount1(event) {
		...
		if(parseFloat(str) > parseFloat(form.deductable_amount1)) {
			...
			setValidationErrors({...validationErrors, deductable_meet_amount1: undefined });
		}
		setForm({ 
			...form,
			...
			<AppBar position="static" elevation={1}>
				<Toolbar className="flex w-full">
					<Typography variant="subtitle1" color="inherit">
						LOP Insurance {'>'} Deductible Detail
					</Typography>
				</Toolbar>
			</AppBar>
			<Formsy
...
export default LOPDeductibleInfo;
