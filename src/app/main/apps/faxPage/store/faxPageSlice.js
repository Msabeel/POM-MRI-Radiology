...
export const getDocuments = createAsyncThunk('uploadsDocumentApp/uploadDocument/getDocuments', async (routeParams, {getState}) => {
	...
	try {
		...
		const response = await axios.post(API_URL.GET_FINALIZED_EXAM, routeParams, {cancelToken: cancelToken.token, });
...
export const uploadBulkDocumentReact = createAsyncThunk(
	...
	async (document, {dispatch, getState}) => {
		try {
			...
			const response = await axios.post(API_URL.BULK_UPLOAD_DOCUMENT_REACT, {...document, user_id: userId.data.userId});
			const data = response.data;
			return {data, isUploaded: true};
		}
		catch (e) {
...
export const uploadDocumentReact = createAsyncThunk(
	...
	async (document, {dispatch, getState}) => {
		try {
			...
			// 	document.data[i].user_id = userId.data.userId// add user id in every object
			// });
			const response = await axios.post(API_URL.UPLOAD_DOCUMENT_REACT, {...document, user_id: userId.data.userId});
			const data = response.data;
			return {data, isUploaded: true};
			// var data = []
			// return { data, isUploaded: true }
...
const faxPageAdapter = createEntityAdapter({});

export const {selectAll: selectContacts, selectById: selectContactsById} = faxPageAdapter.getSelectors(
	state => state.uploadsDocumentApp.contacts
);

const faxPagelice = createSlice({
	name: 'faxPageApp/fax',
	initialState: faxPageAdapter.getInitialState({
		document: '',
		isSearching: false,
		routeParams: {},
		documentType: [],
	...
	extraReducers: {
		...
		[getDocuments.fulfilled]: (state, action) => {
			...
			faxPageAdapter.setAll(state, data);
			state.examData = data;
...
export default faxPagelice.reducer;
