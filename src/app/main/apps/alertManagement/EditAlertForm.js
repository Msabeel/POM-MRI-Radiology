...
import { editAlertManagementData, setResponseStatus } from './store/AlertManagementSlice';
...
const EditFormDialog = ({
	...
	const [alert, setAlertName] = useState('');
	...
	const handleSubmit = async event => {
		...
		const result = await dispatch(editAlertManagementData(data));

		if (result.isUpdateError !== true) {
			customeNotify('Alert updated successfully!', 'success');
	...
	return (
		<div className="flex flex-col min-h-full sm:border-1 overflow-hidden">
			<Dialog onClose={handleClose} aria-labelledby="simple-dialog-title" open={isOpen}>
				<DialogTitle id="simple-dialog-title">Update an Alert</DialogTitle>
